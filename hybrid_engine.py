...
class OCRFieldMatcher:
    ...
    def _match_address_block(
        ...
        if anchors:
            for anchor in anchors:
                nearby_lines = self._collect_lines_near_anchor(anchor, ocr_lines)
                candidate_windows.extend(
                    self._build_candidate_windows_from_lines(nearby_lines, max_window_size=4)
                )
        else:
            candidate_windows = self._build_candidate_windows_from_lines(ocr_lines, max_window_size=4)
        ...
        # Each component line is also proportionally tightened — e.g. if
        # line 1 is "Practice Address: 123 Main St" the label is trimmed off.
